...
class AccountMove(models.Model):
    ...
    invoice_comission = fields.Boolean('Es factura de comisión?')

    def action_post(self):
        """
        Genera una única línea de comisión por factura con el total global,
        y crea líneas por empleado según reglas definidas.
        """
        res = super(AccountMove, self).action_post()

        if not self.invoice_comission and self.move_type == 'out_invoice':
            ...
            for line in self.invoice_line_ids:
                ...
                # 1. Producto
                if line.product_id and line.product_id.commission_assignment == 'invoice':
                    rules = line.product_id.commission_ids
                    origin_obj = line.product_id
                    amount = origin_obj.amount_comission

                # 2. Categoría
                elif line.product_id.categ_id and line.product_id.categ_id.commission_assignment == 'invoice':
                    rules = line.product_id.categ_id.commission_ids
                    origin_obj = line.product_id.categ_id
                    amount = origin_obj.amount_comission

                # 3. Partner
                elif self.partner_id and self.partner_id.commission_assignment == 'invoice':
